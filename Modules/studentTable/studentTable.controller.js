...
export const createStudent = async (req, res) => {
    try {
        ...
        const newStudent = await Student.create({  // ✅ استبدال studentModel بـ Student
            name: req.body.name,
            age: req.body.age,
...
export const getStudents = catchError(async (req, res) => {
    const allStudents = await Student.find(); // ✅ استبدال studentModel بـ Student
    res.status(200).json(allStudents);
});

// البحث عن طالب بناءً على الاستعلامات
export const getStudentByQuery = catchError(async (req, res) => {
    ...
    const students = await Student.find(query); // ✅ استبدال studentModel بـ Student
    res.status(200).json({
        success: true,
...
export const updateStudentById = catchError(async (req, res) => {
    ...
    const student = await Student.findById(studentId); // ✅ استبدال studentModel بـ Student
...
export const deleteStudentById = catchError(async (req, res) => {
    ...
    await Student.findByIdAndDelete(studentId); // ✅ استبدال studentModel بـ Student
    res.json({
        success: true,
